...
def plotraster(spt, stim, win=[0, 30], ntrials=None, ax=None, height=1.0):
...
def plottrains(trains, win=[0, 30], ntrials=None, height=1.0):
    print "Deprecation: Please use plotRasterTrains insted"
    plotRasterTrains(trains, win, ntrials, height)

def plotRasterTrains(trains, win=[0, 30], ntrials=None, height=1.0):
    """Creates raster plots of spike trains:
    ...
    lines = [plt.vlines(sp, np.ones(len(sp)) * i,
                        np.ones(len(sp)) * (i + height))
...
def plotPSTH(spt, stim, win=[0, 30], bin=0.25, ax=None,
    ...
    if rate:
        psth = psth * 1.0 / len(stim) / bin * 1000.0
...
def plotTrainsPSTH(trains, win, bin=0.25, rate=False, **kwargs):
    ...
    if rate:
        psth = psth * 1.0 / len(trains) / bin * 1000.0
        ax.set_ylabel('firing rate (Hz)')
    else:
        ax.set_ylabel('number of spikes')
    ax.set_xlabel('time (ms)')
